...
static inline int argmax_v_par(Scratch *sc, const float *lo, int V){
...
static void dist_build(Scratch *sc, SampBuf *sb, const float *lo, int V){
...
static int dist_sample(const SampBuf *sb, int V){
    double u = rndu(), cum=0;
    for(int i=0;i<V;i++){ cum+=sb->pbuf[i]; if(cum>=u) return i; }
    for(int i=V-1;i>=0;i--) if(sb->pbuf[i]>0) return i;
    return 0;
}
static int pick_tok(Scratch *sc, const float *lo, int V){
