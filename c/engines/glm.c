 ...
 * AUTONOMO: non usa runtime.h/nn.h — porta i propri kernel (matmul_qt su
 * tensori QT con cache-che-impara), il proprio sampler (ban/topk/topp) e un
 * layer di serving (mux multi-slot, kv-disk, replay). La modularizzazione
 * prevista (P3 del piano refactoring) e' stata valutata e SOSPESA senza un
 * GGUF glm_moe_dsa locale per validare: 5.9k righe / 214 statiche non si
 * spostano alla cieca. Sezione mappa (linee indicative):
 *   ~55-470   runtime minimo + kernel matmul (dot_i4f_avx512, i4_acc512)
 *   ~470-800  matmul idot MM + quant_scratch + matmul_qt (tiled/streaming)
 *   ~800-970  quantizzazione (pack_int4/2) + flag di ricerca (ROUTE_J, ...)
 *   ~1016-1170 config + qt_from_disk + model_init
 *   ~1170-1410 CUDA shard/kv + mmap/wire
 *   ~1407-1795 expert_load + io_uring batch
 *   ~1797-1920 pipe workers (disk-parallel reads)
 *   ~1923-2415 attention (MLA: attn_pipe_prefill, attention_rows)
 *   ~2416-2970 moe (router top-k/topp, cache_route, shared)
 *   ~2989-3550 dense_mlp + pilot/couple prefetch + layer_forward
 *   ~3601-3990 step/spec_decode (ngram + MTP draft)
 *   ~3759-3915 grammar + sampler + stops
 *   ~4001-4720 scoring/replay/text/serve (mux)
 *   ~4889-5210 stats/usage learning (ehit/emap/tiers/hwinfo)
 *   ~5110-5479 pin/wire + RAM capping + main
...
#include <sys/stat.h>                             /* fstat per mmap degli shard (MOTY_MMAP) */
...
static int g_metal_gemm_min=16;   /* MOTY_METAL_GEMM_MIN: min rows to send a matmul_qt GEMM to GPU */
...
/* fmt: vocabolario condiviso WF_* di simd.h (WF_F32=0, WF_I8=1, WF_I4=2,
 * WF_I2=3, WF_I4G=4). q4 ospita sia int4 che int2 packed (discriminati da fmt,
 * non dal puntatore). gs=group size (0=per-row, 128=grouped per WF_I4G). */
typedef struct {
...
#ifdef MOTY_CUDA
    MotyCudaTensor *kv_b_shard[MOTY_CUDA_MAX_DEVICES];
    int shard_h0[MOTY_CUDA_MAX_DEVICES],shard_hn[MOTY_CUDA_MAX_DEVICES],n_kv_b_shard;
...
static int g_cuda_devices[MOTY_CUDA_MAX_DEVICES], g_cuda_ndev, g_cuda_rr;
static int64_t g_cuda_dense_projected[MOTY_CUDA_MAX_DEVICES];
static void qt_cuda_reset(QT *t){
    if(t->cuda){ moty_cuda_tensor_free(t->cuda); t->cuda=NULL; }
    t->cuda_failed=0;
}
static int qt_cuda_upload(QT *t){
    const void *weights = t->fmt==0 ? (const void*)t->qf
                        : t->fmt==1 ? (const void*)t->q8 : (const void*)t->q4;
    return moty_cuda_tensor_upload(&t->cuda,weights,t->s,t->fmt,t->I,t->O,t->cuda_device);
}
static int qt_cuda_update(QT *t){
    const void *weights=t->fmt==0?(const void*)t->qf:
                        t->fmt==1?(const void*)t->q8:(const void*)t->q4;
    return moty_cuda_tensor_update(t->cuda,weights,t->s);
}
static void cuda_stats_print(void){
    size_t n=0,b=0; moty_cuda_stats(-1,&n,&b);
    fprintf(stderr,"[CUDA] resident set: %zu tensors, %.2f GB VRAM\n",n,b/1e9);
    if(g_cuda_ndev>1) for(int i=0;i<g_cuda_ndev;i++){
        moty_cuda_stats(g_cuda_devices[i],&n,&b);
    ...
    moty_cuda_group_stats(&calls,&experts,&rows,&h2d,&kernel,&d2h);
    if(calls) fprintf(stderr,"[CUDA] expert groups: %llu call, %llu expert, %llu righe "
        ...
        getenv("MOTY_CUDA_PROFILE")?"; timing sotto":"");
    if(calls&&getenv("MOTY_CUDA_PROFILE")) fprintf(stderr,
...
static int parse_cuda_devices(const char *list, int *out){
    ...
    while(*p){
        ...
        if(end==p||v<0||v>INT_MAX||n>=MOTY_CUDA_MAX_DEVICES) return 0;
 ...
 * this keeps moty dependency-free while removing one team launch/expert. */
...
static int g_no_fused_pair=0;  /* MOTY_NO_FUSED_PAIR=1: disable the gate+up kernel fusion
...
/* simd.h definisce gia' IDOT_KERNEL per i motori densi; glm lo ridefinisce per
 * aggiungere il caso neon-i8mm (il driver tiled SMMLA di glm.c, unico ad usarlo)
 * e allinearlo a sve2-svdot (i dot 1-D ora vengono da simd.h, che su armv9 usa svdot). */
#undef IDOT_KERNEL
#if defined(__ARM_FEATURE_SVE2) && defined(__ARM_FEATURE_SVE)
#define IDOT_KERNEL "sve2-svdot"
...
static void matmul_qt_ex(float *y, const float *x, QT *w, int S, int allow_idot){
    ...
    if(g_metal_enabled && S>=g_metal_gemm_min && !spec_pinned() && (w->fmt==1||w->fmt==2) && !omp_in_parallel()){
        ...
        if(moty_metal_gemm(y,x,wp,w->s,w->fmt,S,w->I,w->O)) return;
    }
#endif
#ifdef MOTY_CUDA
    ...
    if(g_cuda_enabled && w->cuda_eligible && !w->cuda_failed && !omp_in_parallel()){
        ...
        if(moty_cuda_matmul(&w->cuda,y,x,weights,w->s,w->fmt,S,w->I,w->O,w->cuda_device)) return;
        w->cuda_failed=1;
...
/* COUPLE=<.moty_pairs>: coupling-scored cross-layer prefetch. The routing of layer L
...
static void *qalloc(size_t n){
    ...
    if(g_metal_enabled){ void *p; size_t r=(n+16383)&~(size_t)16383;
        ...
        moty_metal_register(p,r); return p; }
#endif
    return malloc(n);
}
...
static void layer_cuda_shard_kvb(Layer *l,int H,int Q,int V){
    ...
    for(int d=0,h0=0;d<g_cuda_ndev;d++){
        ...
        if(!moty_cuda_tensor_upload(&l->kv_b_shard[d],part,scale,l->kv_b.fmt,l->kv_b.I,rows,g_cuda_devices[d]))return;
...
static void model_init(Model *m, const char *snap, int cap, int ebits, int dbits){
    ...
    for(int i=0;i<c->n_layers;i++){
        ...
        if(getenv("MOTY_CUDA_ATTN_SHARD")&&atoi(getenv("MOTY_CUDA_ATTN_SHARD")))
...
/* MOTY_MMAP=1: gli expert diventano VISTE dentro mmap dei file safetensors (niente pread,
 ...
 * qt_wire_mmap() (also further down, used by pin_wire()'s MOTY_MMAP path) needs
...
#ifdef MOTY_METAL
            if(g_metal_enabled) moty_metal_register(p,len);
#endif
        }
    }
#endif
...
static int expert_load(Model *m, int layer, int eid, ESlot *s, int fatal){
    ...
    if(!s->slab || wtot+8192 > s->slab_cap){
        ...
        if(s->slab && g_metal_enabled) moty_metal_unregister(s->slab);
        ...
        s->slab_cap=need;
        if(g_metal_enabled) moty_metal_register(s->slab,need);
#else
        compat_aligned_free(s->slab);
    ...
    if(!s->fslab || ftot > s->fslab_cap){
        ...
        if(s->fslab && g_metal_enabled) moty_metal_unregister(s->fslab);
        free(s->fslab);
        ...
        s->fslab_cap=ftot;
        if(g_metal_enabled) moty_metal_register(s->fslab,fb);
#else
        free(s->fslab);
...
static int uring_batch_init(UringBatch *b){
    ...
    if(moty_uring_init(&b->ring,URING_REQ_MAX)) return -1;
...
static int uring_add_read(UringBatch *b,int li,int fd,void *buf,size_t len,
    ...
    if(moty_uring_prep_read(&b->ring,fd,buf,len,off,(uint64_t)ri+1)) return -1;
    b->load[li].pending++;
    return 0;
}
...
static int uring_load_add(UringBatch *b,Model *m,int layer,int eid,ESlot *s,int fatal){
    ...
    if(!s->slab || wtot+8192>s->slab_cap){
        ...
        s->slab_cap=need; if(g_metal_enabled) moty_metal_register(s->slab,need);
#else
        compat_aligned_free(s->slab);
    ...
    if(!s->fslab || ftot>s->fslab_cap){
        ...
        s->fslab_cap=ftot; if(g_metal_enabled) moty_metal_register(s->fslab,fb);
#else
...
static void uring_reap(UringBatch *b){
    ...
    while(moty_uring_peek(&b->ring,&cqe)){
...
static int uring_submit_batch(UringBatch *b){
    if(moty_uring_enter(&b->ring,0)<0) return -1;
...
static int uring_wait_load(UringBatch *b,int li){
    ...
    while(!l->done){
        ...
        if(moty_uring_enter(&b->ring,1)<0) return uring_load_error(l,errno,"io_uring wait");
    }
    return l->error?-1:0;
}
...
#ifdef MOTY_CUDA
static int g_cuda_pipe=0;   /* MOTY_CUDA_PIPE=1: prefill attention chain resident on the layer home device */
...
static int kv_dev_sync(Model *m, Layer *l, int layer, int upto){
    ...
    if(!m->kv_dev_L[layer]){
        m->kv_dev_L[layer]=(float*)moty_cuda_pipe_alloc(dev,(size_t)m->max_t*kvl*4);
        m->kv_dev_R[layer]=(float*)moty_cuda_pipe_alloc(dev,(size_t)m->max_t*R*4);
        m->kv_dev_valid[layer]=0;
        if(!m->kv_dev_L[layer]||!m->kv_dev_R[layer]) return 0;
    }
    int v=m->kv_dev_valid[layer];
    if(v<upto){
        if(!moty_cuda_pipe_upload(dev,m->kv_dev_L[layer]+(size_t)v*kvl,
            moty_kv_row(m->kv->Lc[layer],v,kvl),(size_t)(upto-v)*kvl*4)||
           !moty_cuda_pipe_upload(dev,m->kv_dev_R[layer]+(size_t)v*R,
            moty_kv_row(m->kv->Rc[layer],v,R),(size_t)(upto-v)*R*4)) return 0;
...
static int attn_pipe_prefill(Model *m, Layer *l, int layer, const float *x, int x_is_dev,
    ...
    float *xd =x_is_dev?(float*)x:moty_cuda_pipe_scratch(dev,0,xb);
    float *qrd=moty_cuda_pipe_scratch(dev,1,qrb);
    float *qd =moty_cuda_pipe_scratch(dev,2,qb),  *cd =moty_cuda_pipe_scratch(dev,3,cb);
    float *ld_=moty_cuda_pipe_scratch(dev,4,lb),  *rd =moty_cuda_pipe_scratch(dev,5,rb);
    float *w1 =moty_cuda_pipe_scratch(dev,6,(size_t)ql*4);
    float *w2 =moty_cuda_pipe_scratch(dev,7,(size_t)kvl*4);
    chost=(float*)malloc(cb);
    if(!xd||!qrd||!qd||!cd||!ld_||!rd||!w1||!w2||!chost) goto done;
    if((!x_is_dev&&!moty_cuda_pipe_upload(dev,xd,x,xb))||
       !moty_cuda_pipe_upload(dev,w1,l->q_a_ln,(size_t)ql*4)||
       !moty_cuda_pipe_upload(dev,w2,l->kv_a_ln,(size_t)kvl*4)) goto done;
    /* proiezioni + norme + rope, tutto sul device */
    if(!moty_cuda_pipe_gemm(l->q_a.cuda,qrd,xd,S)) goto done;
    if(!moty_cuda_pipe_rmsnorm(dev,qrd,qrd,w1,S,ql,c->eps)) goto done;
    if(!moty_cuda_pipe_gemm(l->q_b.cuda,qd,qrd,S)) goto done;
    if(!moty_cuda_pipe_rope_base(dev,qd,pos_base,S*H,qh,c->qk_nope,R,H,c->theta)) goto done;
    if(!moty_cuda_pipe_gemm(l->kv_a.cuda,cd,xd,S)) goto done;
    if(!moty_cuda_pipe_rmsnorm_s(dev,cd,cd,w2,S,kvl,c->eps,kvl+R,kvl+R)) goto done;
    if(!moty_cuda_pipe_rope_base(dev,cd,pos_base,S,kvl+R,kvl,R,1,c->theta)) goto done;
    /* cache latente [T,kvl] + rot [T,R] contigue: righe vecchie da host, nuove da cd */
    if(old>0){
        if(!moty_cuda_pipe_upload(dev,ld_,moty_kv_row(m->Lc[layer],st0,kvl),(size_t)old*kvl*4)||
           !moty_cuda_pipe_upload(dev,rd,moty_kv_row(m->Rc[layer],st0,R),(size_t)old*R*4)) goto done;
    }
    if(!moty_cuda_pipe_copy2d(dev,ld_+(size_t)old*kvl,kvl,cd,kvl+R,kvl,S)) goto done;
    if(!moty_cuda_pipe_copy2d(dev,rd+(size_t)old*R,R,cd+kvl,kvl+R,R,S)) goto done;
    /* KV host resta canonica: scarica i record nuovi (gia' normati+ropati) */
    if(!moty_cuda_pipe_download(dev,cd,chost,cb)) goto done;
    for(int s=0;s<S;s++){
        memcpy(moty_kv_row(m->Lc[layer],pos_base+s,kvl),chost+(size_t)s*(kvl+R),kvl*4);
        memcpy(moty_kv_row(m->Rc[layer],pos_base+s,R),chost+(size_t)s*(kvl+R)+kvl,R*4);
    }
    ...
    if(out_dev && l->n_kv_b_shard>1 &&
       getenv("MOTY_CUDA_PIPE_SHARD") && atoi(getenv("MOTY_CUDA_PIPE_SHARD"))){
        ...
        float *ctx_full=moty_cuda_pipe_scratch(dev,16,ctxb);
        float *stage=moty_cuda_pipe_scratch(dev,17,stage_one*n);
        ...
        if(ok_sh){
            ...
            for(int d2=0;d2<n;d2++){
                ...
                int sdev=moty_cuda_tensor_device(l->kv_b_shard[d2]);
                float *st=stage+(size_t)d2*(stage_one/4);
                size_t qsb=(size_t)S*hn*qh*4, csb=(size_t)S*hn*vh*4;
                float *qs_r=moty_cuda_pipe_scratch(sdev,18,qsb);
                float *ld_r=moty_cuda_pipe_scratch(sdev,19,(size_t)T*kvl*4);
                float *rr_r=moty_cuda_pipe_scratch(sdev,20,(size_t)T*R*4);
                float *cx_r=moty_cuda_pipe_scratch(sdev,21,csb);
                int okd=qs_r&&ld_r&&rr_r&&cx_r;
                /* slice di q: [S,H,qh] -> [S,hn,qh] contigua sul device di casa */
                okd=okd&&moty_cuda_pipe_copy2d(dev,st,hn*qh,qd+(size_t)h0*qh,H*qh,hn*qh,S);
                okd=okd&&moty_cuda_pipe_peer_copy(sdev,qs_r,dev,st,qsb);
                okd=okd&&moty_cuda_pipe_peer_copy(sdev,ld_r,dev,ld_,(size_t)T*kvl*4);
                okd=okd&&moty_cuda_pipe_peer_copy(sdev,rr_r,dev,rd,(size_t)T*R*4);
                okd=okd&&moty_cuda_attention_absorb_batch_dev(l->kv_b_shard[d2],cx_r,qs_r,ld_r,rr_r,
                        S,hn,c->qk_nope,R,vh,kvl,T,c->attn_scale);
                okd=okd&&moty_cuda_pipe_peer_copy(dev,st,sdev,cx_r,csb);
                okd=okd&&moty_cuda_pipe_copy2d(dev,ctx_full+(size_t)h0*vh,H*vh,st,hn*vh,hn*vh,S);
                ok_sh&=okd;
            }
        }
        if(ok_sh){
            ok=moty_cuda_pipe_gemm(l->o.cuda,out_dev,ctx_full,S)&&moty_cuda_pipe_sync(dev);
        } else ok=0;
        if(!ok)
            ok=moty_cuda_attention_project_batch_dev_out(l->kv_b.cuda,l->o.cuda,out_dev,qd,ld_,rd,
                S,H,c->qk_nope,R,c->v_head,kvl,T,c->attn_scale);
    } else
#endif
    ok=out_dev?moty_cuda_attention_project_batch_dev_out(l->kv_b.cuda,l->o.cuda,out_dev,qd,ld_,rd,
            S,H,c->qk_nope,R,c->v_head,kvl,T,c->attn_scale)
              :moty_cuda_attention_project_batch_dev(l->kv_b.cuda,l->o.cuda,out,qd,ld_,rd,
            S,H,c->qk_nope,R,c->v_head,kvl,T,c->attn_scale);
    m->t_acore+=now_s()-t0;
done:
...
static void attention_rows(Model *m, Layer *l, int layer, float *x, int S, int pos_base,
    ...
    if(g_metal_enabled && !kvs && S<=4 && (g_absorb==1||(g_absorb<0&&S<=4)) && m->kv_start[layer]==0
       ...
       && c->qk_rope==64 && vh==256 && l->kv_b.fmt==2){
        ...
        if(!sel_active){
            ...
            int ok = moty_metal_attn_decode(x,
    ...
    if(!pipe_done) for(int s=0;s<S;s++){
        ...
        float *Ldst=moty_kv_row(ks->Lc[layer],pos,c->kv_lora);
        float *Rdst=moty_kv_row(ks->Rc[layer],pos,c->qk_rope);
#ifdef MOTY_CUDA
    ...
    if(m->has_dsa && layer<c->n_layers && ((!kvs && m->kv_start[layer]==0) || kvs)){
        ...
        if(c->idx_type[layer]){
            ...
            for(int s=0;s<S;s++){
                KVState *ks=kvs?kvs[s]:m->kv;
                int pos=positions?positions[s]:pos_base+s;
                float *kd=moty_kv_row(ks->Ic[layer],pos,hd);
            ...
            for(int s=0;s<S;s++){
                ...
                float *isc=falloc(nk);
                for(int t=0;t<nk;t++){
                    const float *kt=moty_kv_row(ks->Ic[layer],t,hd);
                    float a=0;
    ...
    int cuda_absorb=0;
#ifdef MOTY_CUDA
    cuda_absorb=layer<c->n_layers&&!kvs&&g_cuda_enabled&&getenv("MOTY_CUDA_ATTN")&&
                atoi(getenv("MOTY_CUDA_ATTN"))&&c->kv_lora<=512;
#endif
    ...
    if(absorb && c->kv_lora<=512){
        ...
        if(cuda_absorb&&l->n_kv_b_shard>1){
            ...
            for(int d=0;d<n;d++)ok&=moty_cuda_attention_absorb_batch(l->kv_b_shard[d],
                cs+(int64_t)l->shard_h0[d]*S*vh,qs+(int64_t)l->shard_h0[d]*S*qh,
                moty_kv_row(m->Lc[layer],st0,kvl),moty_kv_row(m->Rc[layer],st0,c->qk_rope),
           ...
           qt_cuda_upload(&l->kv_b)&&qt_cuda_upload(&l->o)){
            ...
            cuda_core=cuda_projected=moty_cuda_attention_project_batch(l->kv_b.cuda,l->o.cuda,out,Q,
                moty_kv_row(m->Lc[layer],st0,kvl),moty_kv_row(m->Rc[layer],st0,c->qk_rope),
                S,H,c->qk_nope,c->qk_rope,vh,kvl,nt,c->attn_scale);
        } else if(S<=4&&g_cuda_enabled&&getenv("MOTY_CUDA_ATTN")&&atoi(getenv("MOTY_CUDA_ATTN"))&&
           l->kv_b.cuda_eligible&&qt_cuda_upload(&l->kv_b)){
            ...
            for(int s=0;s<S&&cuda_core;s++){
                ...
                if(g_cuda_pipe&&ks==m->kv&&layer<c->n_layers&&kv_dev_sync(m,l,layer,pos+1))
                    cuda_core=moty_cuda_attention_absorb_kvdev(l->kv_b.cuda,ctx+(int64_t)s*H*vh,
                ...
                if(!cuda_core)
                    cuda_core=moty_cuda_attention_absorb(l->kv_b.cuda,ctx+(int64_t)s*H*vh,
                        Q+(int64_t)s*H*qh,moty_kv_row(ks->Lc[layer],st0,kvl),
                        moty_kv_row(ks->Rc[layer],st0,c->qk_rope),H,c->qk_nope,c->qk_rope,
                        vh,kvl,nt,c->attn_scale);
            }
        }
#endif
        ...
        for(int s=0;s<S;s++) for(int h=0;h<H;h++){
            ...
            for(int jj=0;jj<nt;jj++){ int t = tlist ? tlist[jj] : st0+jj;
                const float *Lt=moty_kv_row(ks->Lc[layer],t,kvl);
                const float *kr=moty_kv_row(ks->Rc[layer],t,c->qk_rope);
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out, int with_shared){
    ...
    for(int base=0;base<nu;base+=64){
        ...
        int is_miss[64]={0}; MotyMetalMoeHandle *mh=NULL;
        ...
        if(g_metal_enabled){
            ...
            if(nbb>0){
                double t0=now_s();
                mh=moty_metal_moe_block_begin(nbb,D,I,mfmt,MG,MU,MD,MGS,MUS,MDS,mxg,xoffb,nrb,mrows,mrw);
                m->t_emm += now_s()-t0;
        ...
        if(g_metal_enabled){
            ...
            if(nbb>0){
                double t0=now_s();
                if(moty_metal_moe_block(nbb,D,I,mfmt,MG,MU,MD,MGS,MUS,MDS,mxg,xoffb,nrb,mrows,mrw,out,S)) cpu_miss=0;
                m->t_emm += now_s()-t0;
            } else cpu_miss=0;
            if(mh){ double t0=now_s();
                if(moty_metal_moe_block_end(mh,out)){ if(mh_shared) shared_on_gpu=1; }
                else cpu_res=1;
        ...
        for(int j=0;j<nb;j++){ int eid=uniq[base+j]; ESlot *e=use[j];
            ...
            if(!group_enabled && g_cuda_enabled && e->g.cuda_eligible && e->u.cuda_eligible &&
               ...
               moty_cuda_expert_mlp(e->g.cuda,e->u.cuda,e->d.cuda,hh,xg,nr)){
...
#ifdef MOTY_CUDA
        MotyCudaTensor *dev_g[MOTY_CUDA_MAX_DEVICES][64],*dev_u[MOTY_CUDA_MAX_DEVICES][64];
        MotyCudaTensor *dev_d[MOTY_CUDA_MAX_DEVICES][64];
        int dev_rows[MOTY_CUDA_MAX_DEVICES][64],dev_which[MOTY_CUDA_MAX_DEVICES][64];
        int dev_nc[MOTY_CUDA_MAX_DEVICES]={0},dev_total[MOTY_CUDA_MAX_DEVICES]={0};
        int dev_off[MOTY_CUDA_MAX_DEVICES]={0},dev_ok[MOTY_CUDA_MAX_DEVICES]={0};
        ...
        for(int di=0;di<g_cuda_ndev;di++) if(dev_nc[di])
            dev_ok[di]=moty_cuda_expert_group(dev_g[di],dev_u[di],dev_d[di],dev_rows[di],dev_nc[di],
        ...
        for(int di=0;di<g_cuda_ndev;di++){
            ...
            for(int q=0;q<dev_nc[di];q++){
                ...
                if(!dev_ok[di]){
                    ...
                    if(!moty_cuda_expert_mlp(e->g.cuda,e->u.cuda,e->d.cuda,hh,xg,nr)){
...
#endif
#ifdef MOTY_CUDA
    int shared_min=getenv("MOTY_CUDA_SHARED_W4A16_MIN_ROWS")?
        atoi(getenv("MOTY_CUDA_SHARED_W4A16_MIN_ROWS")):32;
    ...
    if(shared_cuda==0&&S>=shared_min&&!l->shared_w4a16_failed&&!omp_in_parallel()&&g_cuda_enabled&&
       ...
       getenv("MOTY_CUDA_SHARED_W4A16")&&atoi(getenv("MOTY_CUDA_SHARED_W4A16"))&&
       qt_cuda_upload(&l->sh_gate)&&qt_cuda_upload(&l->sh_up)&&qt_cuda_upload(&l->sh_down)){
        shared_cuda=moty_cuda_shared_mlp_w4a16(l->sh_gate.cuda,l->sh_up.cuda,
...
/* parse .moty_pairs (see tools/route_pairs.py): "MOTYPAIRS 1 <n>" then
...
static void couple_load(Model *m, const char *path){
    ...
    if(fscanf(f,"%15s %d %ld",magic,&ver,&n)!=3 || strcmp(magic,"MOTYPAIRS") || ver!=1){
...
static int pipe_layer_sparse(Model *m, Layer *l, int li, float *x_dev, int S, int pos_base,
    ...
    float *w_in =moty_cuda_pipe_scratch(dev,8,(size_t)D*4);
    float *w_post=moty_cuda_pipe_scratch(dev,9,(size_t)D*4);
    float *nrm_d=moty_cuda_pipe_scratch(dev,10,xb);
    float *y_d  =moty_cuda_pipe_scratch(dev,11,xb);
    float *sg_d =moty_cuda_pipe_scratch(dev,12,(size_t)S*sI*4);
    float *su_d =moty_cuda_pipe_scratch(dev,13,(size_t)S*sI*4);
    float *snap =moty_cuda_pipe_scratch(dev,14,xb);
    if(!w_in||!w_post||!nrm_d||!y_d||!sg_d||!su_d||!snap) return 0;
    if(!moty_cuda_pipe_peer_copy(dev,snap,dev,x_dev,xb)) return 0;   /* snapshot per il fallback */
    if(!moty_cuda_pipe_upload(dev,w_in,l->in_ln,(size_t)D*4)||
       !moty_cuda_pipe_upload(dev,w_post,l->post_ln,(size_t)D*4)) return 0;
    double ta=now_s();
    if(!moty_cuda_pipe_rmsnorm(dev,nrm_d,x_dev,w_in,S,D,c->eps)) return 0;
    ...
    if(m->has_dsa && li<c->n_layers && m->kv_start[li]==0 && c->idx_type[li]){
        if(!moty_cuda_pipe_download(dev,nrm_d,nrm_host,xb)) return 0;
        int nh=c->index_nh, hd=c->index_hd; (void)nh;
        for(int s=0;s<S;s++){
            int pos=pos_base+s;
            float *kd=moty_kv_row(m->kv->Ic[li],pos,hd);
    ...
    if(!moty_cuda_pipe_add(dev,x_dev,y_d,(size_t)S*D)) return 0;         /* prima mutazione */
    if(!moty_cuda_pipe_rmsnorm(dev,nrm_d,x_dev,w_post,S,D,c->eps)) return 0;
    if(!moty_cuda_pipe_download(dev,nrm_d,nrm_host,xb)) return 0;
    m->t_attn+=now_s()-ta;
    ...
    double te=now_s();
    if(!moty_cuda_pipe_gemm(l->sh_gate.cuda,sg_d,nrm_d,S)) return 0;
    if(!moty_cuda_pipe_gemm(l->sh_up.cuda,su_d,nrm_d,S)) return 0;
    if(!moty_cuda_pipe_silu_mul(dev,sg_d,su_d,(size_t)S*sI)) return 0;
    if(!moty_cuda_pipe_gemm(l->sh_down.cuda,y_d,sg_d,S)) return 0;
    if(!moty_cuda_pipe_add(dev,x_dev,y_d,(size_t)S*D)) return 0;  /* shared residual (async) */
    ...
    te=now_s();
    if(!moty_cuda_pipe_upload(dev,y_d,out_host,xb)) return 0;     /* sync: waits for moe */
    if(!moty_cuda_pipe_add(dev,x_dev,y_d,(size_t)S*D)) return 0;  /* routed residual (async) */
    ...
    if(g_metal_enabled && !kvs && S<=4 && li<c->n_layers && l->sparse
       ...
       && c->n_experts==256 && c->topk==8 && c->n_shared==1 && c->moe_inter==2048){
        ...
        if(!sel_active){
            ...
            double ta0=now_s();
            #define WP_(q) ((q).fmt==1?(const void*)(q).q8:(const void*)(q).q4)
            int ok = moty_metal_layer_decode(x, l->in_ln, l->post_ln,
                WP_(l->q_a), l->q_a.s, l->q_a.fmt, l->q_a_ln,
                WP_(l->q_b), l->q_b.s, l->q_b.fmt,
                WP_(l->kv_a), l->kv_a.s, l->kv_a.fmt, l->kv_a_ln,
                WP_(l->kv_b), l->kv_b.s, l->kv_b.fmt,
...
static void layers_forward_rows(Model *m, float *x, int S, int pos_base,
     ...
     * MOTY_CUDA_PIPE_S_MIN overrides for anyone who wants to measure. */
    float *x_dev=NULL; int x_dev_on=-1;
    size_t xb=(size_t)S*(size_t)D*4;
    int pipe_s_min = getenv("MOTY_CUDA_PIPE_S_MIN") ? atoi(getenv("MOTY_CUDA_PIPE_S_MIN"))
                                                     : (g_cuda_ndev<=1 ? 1 : 8);
    ...
    for(int i=0;i<c->n_layers;i++){
        ...
        if(pipe2 && l->sparse && i<c->n_layers &&
           ...
           l->kv_a.cuda_device==l->kv_b.cuda_device&&l->o.cuda_device==l->kv_b.cuda_device){
            ...
            float *dst=moty_cuda_pipe_scratch(dev,15,xb);
            if(dst){
                if(x_dev_on<0) ok=moty_cuda_pipe_upload(dev,dst,x,xb);
                else if(x_dev_on!=dev){ ok=moty_cuda_pipe_peer_copy(dev,dst,x_dev_on,x_dev,xb); }
                else dst=x_dev;
                if(ok){
                    ...
                    moty_cuda_pipe_peer_copy(dev,x_dev,dev,moty_cuda_pipe_scratch(dev,14,xb),xb);
                    moty_cuda_pipe_download(dev,x_dev,x,xb);
                    x_dev_on=-1;
                }else x_dev_on=-1;
            }
        } else if(x_dev_on>=0){                 /* layer fuori pipe: il residuo torna a casa */
            moty_cuda_pipe_download(x_dev_on,x_dev,x,xb);
            x_dev_on=-1;
        }
#endif
...
#ifdef MOTY_CUDA
    if(x_dev_on>=0) moty_cuda_pipe_download(x_dev_on,x_dev,x,xb);
#endif
    free(nrm); free(tmp);
}
...
static void kv_alloc(Model *m, int max_t){
    ...
    if(m->kv_dev_L) for(int i=0;i<c->n_layers+1;i++){    /* dimensioni cambiate: ombra da rifare */
        if(m->kv_dev_L[i]){ moty_cuda_pipe_free(m->L[i<c->n_layers?i:0].kv_b.cuda_device,m->kv_dev_L[i]); m->kv_dev_L[i]=NULL; }
        if(m->kv_dev_R[i]){ moty_cuda_pipe_free(m->L[i<c->n_layers?i:0].kv_b.cuda_device,m->kv_dev_R[i]); m->kv_dev_R[i]=NULL; }
        m->kv_dev_valid[i]=0;
    }
#endif
    if(k->Lc){ for(int i=0;i<c->n_layers+1;i++){
#ifdef MOTY_METAL
        if(g_metal_enabled){ moty_metal_unregister(k->Lc[i]); moty_metal_unregister(k->Rc[i]); }
#endif
    ...
    for(int i=0;i<NR;i++){ k->Lc[i]=falloc((int64_t)max_t*c->kv_lora);
        ...
        if(g_metal_enabled){
            ...
            k->Lc[i]=lp; k->Rc[i]=rp;
            moty_metal_register(k->Lc[i],lb); moty_metal_register(k->Rc[i],rb);
        }
#endif
    }
...
static void profile_print(Model *m, double elapsed){
    ...
    if(g_metal_enabled){ uint64_t ok=0,fb=0,ex=0; double su=0,gp=0,sc=0;
        moty_metal_moe_counts(&ok,&fb,&ex); moty_metal_moe_times(&su,&gp,&sc);
        { uint64_t aok=0; double aw=0,ak=0; moty_metal_attn_counts(&aok,&aw,&ak);
          if(aok){ double ks=0,gs=0; moty_metal_attn_lat(&ks,&gs);
        ...
        printf("METAL: blocchi GPU %llu | fallback CPU %llu | expert su GPU %llu | setup %.2fs gpu-wall %.2fs (kernel %.2fs) scatter %.2fs\n",
               (unsigned long long)ok,(unsigned long long)fb,(unsigned long long)ex,su,gp,moty_metal_moe_kernel_time(),sc); }
#endif
}
...
/* modalita' SERVE (per la CLI 'moty'): carica il modello UNA volta, poi CHAT conversazionale.
 ...
 * Upstream fa AUTOPIN allo START (dalla storia .moty_usage). Questo aggiunge un re-pin
 ...
 * map separata decade a ogni passata: la storia persistente .moty_usage resta intatta.
 * EN: upstream AUTOPINs at START (from .moty_usage). This adds a between-turns re-pin: at
 ...
 * persistent .moty_usage intact while adapting to the current workload. */
...
static void repin_pass_limit(Model *m,int limit){
    ...
    for(int b=0;b<nb;b++){
        ...
        int64_t old_gpu=gpu ? (int64_t)moty_cuda_tensor_bytes(s->g.cuda)
                             +(int64_t)moty_cuda_tensor_bytes(s->u.cuda)
                             +(int64_t)moty_cuda_tensor_bytes(s->d.cuda) : 0;
#endif
        double t0=now_s();
        ...
        if(gpu){                                  /* refresh the same VRAM slot now, not lazily */
            if(qt_cuda_upload(&s->g) && qt_cuda_upload(&s->u) && qt_cuda_upload(&s->d)){
                int64_t now_gpu=(int64_t)moty_cuda_tensor_bytes(s->g.cuda)
                               +(int64_t)moty_cuda_tensor_bytes(s->u.cuda)
                               +(int64_t)moty_cuda_tensor_bytes(s->d.cuda);
 ...
 * costa ~182 KB/token. File <SNAP>/.moty_kv append-only: header (magic + dimensioni +
...
static int kv_disk_load(Model *m, int *hist, int maxctx){
    ...
    if(fread(mg,1,8,f)!=8 || memcmp(mg,KV_MAGIC,8) || fread(h,4,8,f)!=8 ||
       h[0]!=w[0]||h[1]!=w[1]||h[2]!=w[2]||h[3]!=w[3]||h[4]!=w[4]||h[5]!=w[5]){
        fprintf(stderr,"[KV] ignoring .moty_kv from a different model or version\n"); fclose(f); return 0; }
    int nrec=h[6];
...
static void serve_ctx_init(Model *m, ServeCtx *s, const char *snap, int slot, int maxctx){
    ...
    s->first=1;
    if(slot==0) snprintf(s->kv.disk_path,sizeof(s->kv.disk_path),"%s/.moty_kv",snap);
    else snprintf(s->kv.disk_path,sizeof(s->kv.disk_path),"%s/.moty_kv.%d",snap,slot);
...
static int mux_submit(Model *m, Tok *T, ServeCtx *ctx, ServeReq *req, int nctx,
    ...
    MotySubmit sub; int valid=moty_submit_parse(line,&sub);
...
static void run_serve(Model *m, const char *snap){
     ...
     * 'moty' matches the sentinels with endswith() and a "^STAT ..." regex,
...
static void hwinfo_emit(Model *m){
    ...
    for(int i=0;i<g_cuda_ndev;i++){
        size_t fr=0,to=0; moty_cuda_mem_info(g_cuda_devices[i],&fr,&to);
...
/* CACHE CHE IMPARA: istogramma d'uso PERSISTENTE in <SNAP>/.moty_usage.
 * Caricato all'avvio (i contatori ripartono dalla storia), salvato a ogni turno:
 * piu' usi moty', meglio l'auto-pin conosce i TUOI expert caldi. */
...
/* HOT-STORE ("il redis del moty'"): carica in RAM, UNA VOLTA e per sempre, i top expert
...
static void pin_load(Model *m, const char *statspath, double gb){
    ...
    double t0=now_s();
#ifdef MOTY_CUDA
    double remaining[MOTY_CUDA_MAX_DEVICES]={0}, placed_b[MOTY_CUDA_MAX_DEVICES]={0};
    int placed_n[MOTY_CUDA_MAX_DEVICES]={0}, gpu_prefix=0;
    ...
    if(g_cuda_enabled&&(g_cuda_expert_gb>0||g_cuda_expert_auto)) for(int i=0;i<g_cuda_ndev;i++){
        ...
        if(moty_cuda_mem_info(g_cuda_devices[i],&free_b,&total_b)){
    ...
    if(g_cuda_enabled && budget>0){
        ...
        for(int a=0;a<gpu_limit && m->gpu_expert_bytes<budget;a++){
            ...
            { ESlot *s=&m->pin[li][slot_of[a]];
                ...
                int tried[MOTY_CUDA_MAX_DEVICES]={0}, placed=0;
                for(int attempt=0;attempt<g_cuda_ndev && !placed;attempt++){
                    ...
                    if(qt_cuda_upload(&s->g) && qt_cuda_upload(&s->u) && qt_cuda_upload(&s->d)){
                        int64_t actual=(int64_t)moty_cuda_tensor_bytes(s->g.cuda)
                                      +(int64_t)moty_cuda_tensor_bytes(s->u.cuda)
                                      +(int64_t)moty_cuda_tensor_bytes(s->d.cuda);
...
static void cap_for_ram(Model *m, double ram_gb, int ebits, int max_ctx){
    ...
    if(floored){
        ...
        if(g_mem_avail_boot>0 && peak > g_mem_avail_boot*1e9 &&
           !(getenv("MOTY_RAM_OVERCOMMIT") && atoi(getenv("MOTY_RAM_OVERCOMMIT")))){
            fprintf(stderr,"[RAM] refusing to start: that peak also exceeds the %.1f GB actually "
                ...
                "(MOTY_RAM_OVERCOMMIT=1 overrides this check).\n", g_mem_avail_boot);
            exit(2);
        }
    }
    if(capmax < m->ecap){
         ...
         * (cap=8 di default in moty): hit 23-28% con decine di GB inutilizzati.
...
/* The user's generation prompt. MOTY_PROMPT is honored on every platform; a bare
 ...
 * cmd's $-metacodes is ignored; set MOTY_PROMPT to pass a real prompt from cmd. */
static const char *moty_user_prompt(void){
    const char *p = getenv("MOTY_PROMPT");
    if(p) return p;
    p = getenv("PROMPT");
#ifdef _WIN32
...
int main(int argc, char **argv){
     ...
     * constructor picks them up. The MOTY_OMP_TUNED sentinel guards the exec so
     * we re-exec at most once. Fully overridable: any explicit OMP_/GOMP_ env the
     * user sets wins (overwrite=0), pre-setting MOTY_OMP_TUNED=1 skips the
     * re-exec entirely (runs with whatever policy the environment already has),
     * and MOTY_NO_OMP_TUNE=1 is a documented kill-switch that disables the whole
     * re-exec + tuning path (distinct from the internal MOTY_OMP_TUNED sentinel).
     *
     * Must remain the FIRST statement in main(): argv is passed verbatim to execv(). */
    if(!getenv("MOTY_OMP_TUNED") && !getenv("MOTY_NO_OMP_TUNE") &&
       !getenv("MOTY_CUDA") && !getenv("MOTY_METAL")){
    ...
    /* glm legge solo il container safetensors pre-quantizzato di moty (pesi +
     * companion .qs): niente percorso GGUF. Lo segnaliamo chiaro invece di
     * cadere silenziosamente su un SNAP sbagliato. EN: glm is safetensors-only. */
    if(getenv("GGUF") && *getenv("GGUF")){fprintf(stderr,"glm: GGUF non supportato (SNAP=<dir container moty>). Usa qwen/gemma per i modelli single-file.\n");return 1;}
    g_nopack = getenv("NOPACK")?1:0;
    g_drop = getenv("DROP")?1:0;
    g_prefetch = getenv("PREFETCH")?atoi(getenv("PREFETCH")):0;
    g_mmap = getenv("MOTY_MMAP")?atoi(getenv("MOTY_MMAP")):0;
    ...
    const char *policy=getenv("MOTY_POLICY"); if(!policy) policy="quality";
    int experimental=!strcmp(policy,"experimental-fast");
    if(strcmp(policy,"quality")&&strcmp(policy,"balanced")&&!experimental){
        fprintf(stderr,"MOTY_POLICY non valida: quality, balanced o experimental-fast\n"); return 2;
    }
    ...
    g_no_fused_pair = getenv("MOTY_NO_FUSED_PAIR")?atoi(getenv("MOTY_NO_FUSED_PAIR")):0;   /* -1 = auto: 3 se MTP, 0 senza */
    ...
    if(g_uring){
#ifdef __linux__
        if(g_mmap){ fprintf(stderr,"URING=1 is incompatible with MOTY_MMAP=1\n"); return 2; }
        g_pipe=1;
        ...
        if(moty_uring_set_workers(&g_ub_pipe.ring,uw) ||
           (g_pilot_real&&moty_uring_set_workers(&g_ub_pilot.ring,uw)))
...
#ifdef MOTY_CUDA
    if(getenv("MOTY_CUDA") && atoi(getenv("MOTY_CUDA"))){
        const char *one=getenv("MOTY_GPU"), *many=getenv("MOTY_GPUS");
        if(one&&many){ fprintf(stderr,"use MOTY_GPU or MOTY_GPUS, not both\n"); return 2; }
        ...
        if(g_cuda_ndev<1){ fprintf(stderr,"invalid MOTY_GPUS: use a list such as 0,1,2\n"); return 2; }
        g_cuda_enabled=moty_cuda_init(g_cuda_devices,g_cuda_ndev);
        if(!g_cuda_enabled){ fprintf(stderr,"[CUDA] requested backend is unavailable\n"); return 2; }
    }
    g_cuda_dense=getenv("CUDA_DENSE")?atoi(getenv("CUDA_DENSE")):0;
    g_cuda_pipe=getenv("MOTY_CUDA_PIPE")?atoi(getenv("MOTY_CUDA_PIPE")):0;
    ...
    if((getenv("MOTY_GPU")||getenv("MOTY_GPUS"))&&!g_cuda_enabled){ fprintf(stderr,"MOTY_GPU(S) requires MOTY_CUDA=1\n"); return 2; }
    if(g_cuda_dense&&!g_cuda_enabled){ fprintf(stderr,"CUDA_DENSE requires MOTY_CUDA=1\n"); return 2; }
    if((g_cuda_expert_gb>0||g_cuda_expert_auto) && !g_cuda_enabled){ fprintf(stderr,"CUDA_EXPERT_GB requires MOTY_CUDA=1\n"); return 2; }
...
#else
    if((getenv("MOTY_CUDA") && atoi(getenv("MOTY_CUDA"))) ||
       getenv("MOTY_GPU") || getenv("MOTY_GPUS") ||
...
#endif
#ifdef MOTY_METAL
    if(getenv("MOTY_METAL") && atoi(getenv("MOTY_METAL"))){
        g_metal_enabled = moty_metal_init();
        ...
        if(getenv("MOTY_METAL_SPIN") && atoi(getenv("MOTY_METAL_SPIN"))){ moty_metal_spin_start(); fprintf(stderr,"[METAL] keep-alive spinner ON\n"); }
        if(getenv("MOTY_METAL_GEMM_MIN")) g_metal_gemm_min=atoi(getenv("MOTY_METAL_GEMM_MIN"));
    }
#else
    if(getenv("MOTY_METAL") && atoi(getenv("MOTY_METAL"))){
    ...
    if(g_draft<0){
         ...
         * always exists on a single 16 GB card. MOTY_CUDA_MTP=1 opts in for
        ...
        int cuda_mtp = getenv("MOTY_CUDA_MTP") ? atoi(getenv("MOTY_CUDA_MTP")) : 0;
    ...
    /* anche su stderr: e' il canale che le UI (moty) mostrano all'utente */
    ...
    if(getenv("PIN")){
        ...
        if(!strcmp(pin,"auto")){
            /* PIN=auto: la storia VIVA <SNAP>/.moty_usage (appesa a ogni turno) batte il
            ...
            snprintf(pauto,sizeof(pauto),"%s/.moty_usage",snap);
            ...
            else { pin=NULL; fprintf(stderr,"[PIN] auto: no .moty_usage or stats.txt in %s yet (no pin this run)\n",snap); }
        }
        if(pin){
    ...
    /* CACHE CHE IMPARA: l'uso degli expert si accumula in <SNAP>/.moty_usage tra le sessioni;
    ...
    { double ram_env = getenv("RAM_GB")?atof(getenv("RAM_GB")):0.0;
      ...
      snprintf(g_usage_path,sizeof(g_usage_path),"%s/.moty_usage",snap);
    ...
    /* modo serve persistente per la CLI 'moty': SERVE=1 */
    if(getenv("SERVE")){
    ...
    const char *user_prompt = moty_user_prompt();   /* ignores cmd.exe's PROMPT template (#271) */
    if(user_prompt){
    ...
    { int maxid=0; for(int i=0;i<nfull;i++) if(full[i]>maxid) maxid=full[i];
      if(m.c.vocab>1000 && maxid<1000 && !getenv("REF_FORCE")){
        fprintf(stderr,
          ...
          "         or:                python moty chat --model <model>\n"
