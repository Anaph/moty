#ifndef MOTY_DECODE_BATCH_H
#define MOTY_DECODE_BATCH_H
...
static inline float *moty_kv_row(float *base, int position, int width)
{
...
static inline int moty_submit_parse(const char *line, MotySubmit *s)
{
    char tail;
    if (!line || !s ||
