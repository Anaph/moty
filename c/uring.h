...
/* Minimal Linux io_uring reader.  Moty owns the ring from one thread, queues
...
static inline unsigned moty_uring_load_acquire(unsigned *p){
    return __atomic_load_n(p,__ATOMIC_ACQUIRE);
}
static inline void moty_uring_store_release(unsigned *p,unsigned v){
    __atomic_store_n(p,v,__ATOMIC_RELEASE);
}

static inline void moty_uring_close(MotyUring *r){
    if(!r) return;
...
static inline int moty_uring_init(MotyUring *r,unsigned entries){
    memset(r,0,sizeof(*r)); r->fd=-1;
    ...
    if(r->p.features&IORING_FEAT_SINGLE_MMAP){
        ...
        if(r->sq_map==MAP_FAILED){ r->sq_map=NULL; moty_uring_close(r); return -1; }
        ...
        if(r->cq_map==MAP_FAILED){ r->cq_map=NULL; moty_uring_close(r); return -1; }
    }
    ...
    if(r->sqes_map==MAP_FAILED){ r->sqes_map=NULL; moty_uring_close(r); return -1; }
...
static inline int moty_uring_set_workers(MotyUring *r,unsigned workers){
...
static inline int moty_uring_prep_read(MotyUring *r,int fd,void *buf,size_t len,
                                       int64_t off,uint64_t user_data){
    if(!len || len>UINT32_MAX){ errno=EINVAL; return -1; }
    unsigned head=moty_uring_load_acquire(r->sq_head);
    ...
    r->sq_array[idx]=idx;
    moty_uring_store_release(r->sq_tail,tail+1);
    return 0;
}

static inline int moty_uring_enter(MotyUring *r,unsigned min_complete){
    for(;;){
        unsigned head=moty_uring_load_acquire(r->sq_head);
        unsigned tail=moty_uring_load_acquire(r->sq_tail);
...
static inline int moty_uring_peek(MotyUring *r,struct io_uring_cqe *out){
    unsigned head=__atomic_load_n(r->cq_head,__ATOMIC_RELAXED);
    unsigned tail=moty_uring_load_acquire(r->cq_tail);
    if(head==tail) return 0;
    *out=r->cqes[head&*r->cq_mask];
    moty_uring_store_release(r->cq_head,head+1);
    return 1;
}
